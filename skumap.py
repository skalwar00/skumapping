...
if st.session_state.user is None:
    if token:
        ...
        # 0.6 sec wait to allow cookie sync from browser
        time.sleep(0.6)
        token_retry = cookie_manager.get(cookie="sb-access-token")
        if token_retry:
            st.rerun()

# --- 5. UI & UTILS FUNCTIONS ---
def login_signup_ui():
...
with t2:
    ...
    if not all_designs:
        ...
        with st.form("cost_up"):
            ...
            if st.form_submit_button("Save Costing"):
                supabase.table("design_costing").upsert({"user_id": u_id, "design_pattern": sel, "landed_cost": new_v}).execute()
                st.cache_data.clear(); st.success(f"Saved: {sel} at ₹{new_v}"); st.rerun()
...
with t4:
    ...
    uploaded_files = st.file_uploader("Upload Myntra Files", type=['csv'], accept_multiple_files=True)
    
    def get_final_cost(sku_name):
        sku = str(sku_name).strip()
        mapped = mapping_dict.get(sku.upper(), sku)
        pattern = get_design_pattern(mapped)
        if pattern in costing_dict: return costing_dict[pattern]
        sku_up = sku.upper()
        if sku_up.startswith('HF'): return 230 if ('CBO' in sku_up or 'COMBO' in sku_up) else 115
        elif sku_up.startswith('PT'): return 330 if ('CBO' in sku_up or 'COMBO' in sku_up) else 165
        return 0
        ...
        if uploaded_files and len(uploaded_files) >= 3:
            ...
            for file in uploaded_files:
                ...
                if 'sale_order_code' in df.columns: flow_df = df
                    ...
                    fname = file.name.lower()
                    if 'reverse' in fname or 'return' in fname: rev_list.append(df)
                    else: fwd_list.append(df)

            if flow_df is not None and sku_df is not None:
                final = pd.merge(flow_df, sku_df[['order release id', 'seller sku code']].drop_duplicates(), 
                                 left_on='sale_order_code', right_on='order release id', how='left')
                
                fwd_combined = pd.concat(fwd_list, ignore_index=True) if fwd_list else pd.DataFrame()
                rev_combined = pd.concat(rev_list, ignore_index=True) if rev_list else pd.DataFrame()

                if not fwd_combined.empty:
                ...
                for col in ['Forward_Amt', 'Reverse_Amt']: final[col] = pd.to_numeric(final[col], errors='coerce').fillna(0)
                final['Net_Settlement'] = final['Forward_Amt'] + final['Reverse_Amt']
                final['Unit_Cost'] = final['seller sku code'].apply(get_final_cost)
                final['Total_Cost'] = final.apply(lambda x: x['Unit_Cost'] if str(x['order_item_status']).lower() == 'delivered' else 0, axis=1)
                final['Net_Profit'] = final['Net_Settlement'] - final['Total_Cost']
                ...
                st.dataframe(final[['sale_order_code', 'seller sku code', 'Net_Settlement', 'Net_Profit']], use_container_width=True)
            else: st.error("Files identification failed.")
