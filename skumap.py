...
def load_all_data(u_id):
    # Sabhi tables se data load karna (Trial Profile ke saath)
...
def generate_4x6_pdf(df):
    ...
    c.drawCentredString(2*INCH, 5.7*INCH, "AAVONI PICKLIST")
    y = 5.4*INCH
    for _, row in df.iterrows():
        if y < 40: c.showPage(); y = 5.7*INCH
        c.setFont("Helvetica", 10)
        c.drawString(20, y, f"{str(row['Master_SKU'])[:25]}")
        c.drawRightString(4*INCH-20, y, f"Qty: {row['Qty']}")
        y -= 20
...
if st.session_state.user is None:
    ...
    # Unpack charo variables (Picklist aur Trail dono ke liye)
    ...
    with st.sidebar:
        ...
        if profile and profile.get('plan_expiry'):
            exp = datetime.strptime(profile['plan_expiry'], '%Y-%m-%d').date()
    ...
    # --- TAB 1: PICKLIST (RE-FIXED) ---
    with tabs[1]:
        st.header("Order Processing")
        files = st.file_uploader("Upload Orders (CSV)", type=["csv"], accept_multiple_files=True, key="pk_files")
        if files:
            orders = []
            for f in files:
                df_c = pd.read_csv(f)
                df_c.columns = [c.lower().strip() for c in df_c.columns]
                sku_col = 'seller sku code' if 'seller sku code' in df_c.columns else next((c for c in df_c.columns if 'sku' in c and 'myntra' not in c), None)
                if sku_col:
                    for s in df_c[sku_col].dropna(): orders.append({'Portal_SKU': str(s).strip().upper(), 'Qty': 1})
            
            if orders:
                df_o = pd.DataFrame(orders)
                if st.button("Generate 4x6 Picklist"):
                    # Mapping connection check
                    ...
                    if not ready.empty:
                        ...
                        st.warning("Mapping not found! Please map SKUs below.")

    # --- TAB 4: MYNTRA (YOUR SCRIPT MODIFIED) ---
    with tabs[4]:
        st.header("Myntra Smart Analyzer 🚀")
        m_files = st.file_uploader("Upload 4-6 Myntra Files", type=['csv'], accept_multiple_files=True, key="my_bulk")
        ...
        def get_myntra_cost(sku_name):
            ...
            return 330 if 'CBO' in sku else 165
            ...
            if len(m_files) >= 4:
                ...
                for f in m_files:
                    ...
                    if 'sale_order_code' in tdf.columns: f_df = tdf
                        ...
                        if 'reverse' in f.name.lower() or val < 0: rev.append(tdf)
                        else: fwd.append(tdf)
