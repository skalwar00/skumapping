...
def get_design_pattern(master_sku):
    ...
    # Remove sizes like -S, -M, -XL, -4XL, etc.
...
if st.session_state.user is None:
    ...
    # --- TAB 2: COSTING MANAGER (WITH DROPDOWN & BLANK ALERTS) ---
    with t2:
        st.header("💰 Design-wise Costing Manager")
        
        # Logic for identifying designs from mapping
        all_master_skus = list(set(mapping_dict.values()))
        all_designs_in_mapping = sorted(list(set([get_design_pattern(s) for s in all_master_skus])))
        missing_costs = [d for d in all_designs_in_mapping if d not in costing_dict]
        
        if missing_costs:
            st.warning(f"⚠️ {len(missing_costs)} Designs ki costing missing hai!")
            with st.expander("Dekhein kaunse designs blank hain"):
                st.write(", ".join(missing_costs))
        
        st.divider()
        st.subheader("Update/Add Costing")
        with st.form("cost_update_form"):
            col1, col2 = st.columns(2)
            # Combine missing first, then existing
            selection_list = missing_costs + [d for d in all_designs_in_mapping if d in costing_dict]
            selected_design = col1.selectbox("Select Design Pattern", options=selection_list)
            
            current_val = costing_dict.get(selected_design, 0.0)
            new_cost = col2.number_input(f"Landed Cost (₹)", min_value=0.0, value=float(current_val))
            
            if st.form_submit_button("Save Costing"):
                if selected_design:
                    supabase.table("design_costing").upsert({
                        "user_id": u_id, 
                        "design_pattern": selected_design, 
                        "landed_cost": new_cost
                    }, on_conflict="user_id, design_pattern").execute()
                    st.success(f"✅ {selected_design} saved!"); st.rerun()
    ...
    with t3:
        st.header("Flipkart Orders P&L Analysis")
        ...
        if uploaded_file:
            try:
                ...
                if sku_col in df.columns and settlement_col in df.columns:
                    ...
                    m1.metric("Total Settlement", f"₹{int(df[settlement_col].sum()):,}")
                    m2.metric("Total Profit", f"₹{int(df['Net_Profit'].sum()):,}")
                    m3.metric("Net Units", f"{df[units_col].sum():,}")
